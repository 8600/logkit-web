  ...
  .collector-box
    .usual-setting
      template(v-for="(item, index) in normal")
        ...
        TextInput.input-item(v-else, :value="item.Default", @input="changeNormalConfig(item.KeyName, $event, index)", :required="item.required", :placeholder="item.placeholder", :label="item.Description")
    ...
    template(v-if="advance.length > 0")
      ...
      .senior-setting(v-show="showAdvance")
        template(v-for="(item, index) in advance")
          ...
          TextInput.input-item(v-else, :value="item.Default", @input="changeAdvanceConfig(item.KeyName, $event, index)", :required="item.required", :placeholder="item.placeholder", :label="item.Description")
</template>
